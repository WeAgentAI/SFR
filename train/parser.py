...
def get_train_args(args: Optional[Union[dict[str, Any], list[str]]] = None) -> _TRAIN_CLS:
    ...
    # Check arguments
    if finetuning_args.stage not in ("sft", "fm"):
        if training_args.predict_with_generate:
            raise ValueError("`predict_with_generate` cannot be set as True except SFT/FM.")

        if data_args.neat_packing:
            raise ValueError("`neat_packing` cannot be set as True except SFT/FM.")

        if data_args.train_on_prompt or data_args.mask_history:
            raise ValueError("`train_on_prompt` or `mask_history` cannot be set as True except SFT/FM.")

    if finetuning_args.stage in ("sft", "fm") and training_args.do_predict and not training_args.predict_with_generate:
