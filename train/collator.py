...
class SFTDataCollatorWith4DAttentionMask(MultiModalDataCollatorForSeq2Seq):
    ...
    def __call__(self, features: list[dict[str, Any]]) -> dict[str, "torch.Tensor"]:
        # Pop FM target embeddings before parent __call__ to avoid tokenizer.pad()
        fm_embeddings = None
        if features and "fm_target_embedding" in features[0]:
            fm_embeddings = [feature.pop("fm_target_embedding") for feature in features]
        ...
        # Re-attach FM embeddings as (B, d) tensor
        if fm_embeddings is not None:
            features["fm_target_embedding"] = torch.tensor(fm_embeddings, dtype=self.compute_dtype)
